...
pub fn check_boundaries(imports: &[ImportRecord]) -> Vec<Violation> {
    ...
    for record in imports {
        ...
        let source_layer = source.map_or(Layer::Unknown, |(_, layer)| layer);

        if !matches!(source_layer, Layer::Client | Layer::Server) {
            continue;
        }
        ...
        let target_layer = target_parsed.map_or(Layer::Unknown, |(_, layer)| layer);
        ...
        match (source_layer, target_layer) {
            ...
            (Layer::Server, Layer::Client) => violations.push(Violation {
                ...
                suggestion:
                    "move reusable logic to shared layer and avoid reverse dependency".to_string(),
            }),
            _ => {}
        }
    }
